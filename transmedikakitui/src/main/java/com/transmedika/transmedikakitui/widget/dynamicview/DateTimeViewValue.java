...
public class DateTimeViewValue extends SingleValueWidget<String> implements View.OnClickListener {
    private Calendar tgl;
    public DateTimeViewValue(Context context) {
        super(context);
    }
    ...
    private void showDialog(View v){
        ...
        DatePickerDialog pickerDialog = new DatePickerDialog(getContext(),R.style.DialogTheme, (viewDate, year, month, dayOfMonth) -> {
            TimePickerDialog timePickerDialog = new TimePickerDialog(getContext(),R.style.DialogTheme, (viewTime, hourOfDay, minute) -> {
                if (tgl == null) {
                    tgl = initCalendar;
                }
            ...
            timePickerDialog.setOnShowListener(dialog -> {
                timePickerDialog.getButton(AlertDialog.BUTTON_NEGATIVE).setTextColor(ContextCompat.getColor(getContext(),R.color.colorPrimary));
                timePickerDialog.getButton(AlertDialog.BUTTON_POSITIVE).setTextColor(ContextCompat.getColor(getContext(),R.color.colorPrimary));
            });
    ...
    void onSetUiFromJawaban(JawabanSingle jawaban) {
        ...
        if (answer != null && !answer.isEmpty()) {
            ...
            tgl.setTime(DateUtil.toDate(answer, pertanyaan.getFormat() != null ? pertanyaan.getFormat() : Constants.DATE_TIME_ZONE_2));
            editText.setText(getValue() != null ? getValue() : "");
        }
    }
    ...
    public String getValue(){
        if (tgl != null) {
            return DateUtil.dateToString(tgl.getTime(),  pertanyaan.getFormat()!=null ? pertanyaan.getFormat() : Constants.DATE_TIME_ZONE_2);
        }
        return null;
    }
