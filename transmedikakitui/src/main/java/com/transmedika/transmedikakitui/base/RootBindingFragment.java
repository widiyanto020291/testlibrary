...
public abstract class RootBindingFragment<VB extends ViewBinding, V extends BaseView, T extends BasePresenter<V>> extends BaseBindingFragment<VB, V, T> {
    ...
    void errorView(String msg, int image){
    ...
    public void hideLoading(boolean usingDialog) {
        if(!usingDialog) {
            if (!isEmpty) {
                ...
                errorView(emptyMsgDefault!=null? emptyMsgDefault:mActivity.getString(R.string.tidak_ada_data_yang_ditampilkan),
    ...
    private void ifHaveToolbar(View parent){
        //kondisional, view loadingnya punya toolbar, karena layout view_main menggunakan coordinator layout, agar toolbar tetap muncul
        /*if(parent.findViewById(R.id.toolbar) !=null){
            Toolbar toolbar = parent.findViewById(R.id.toolbar);
            toolbar.findViewById(R.id.rl_back).setOnClickListener(v -> mActivity.finish());
        }*/
    }
}
